...
- [x] React 기반 초기 앱 구조 구성 (Next.js 14 App Router)
- [x] Mapbox GL JS 연동 및 GeoJSON 오버레이 (구/동 단위)
- [x] 기본 지표(인구 수) 기반 클로로플레스 색상 표현
- [x] 마우스오버 툴팁, 클릭 시 상세 정보 카드 출력 (RankingSidebar)
- [x] 서울시 API 연동 (OpenAPI 실시간 데이터 fetch)
- [x] 검색 기능 (지표 검색 - HierarchicalIndicatorSelector)
- [x] 구/동 단위 레이어 토글 기능
- [ ] PDF / 이미지 저장 기능 (지도 + 범례 포함)
- [x] 모바일 대응 반응형 레이아웃 구성
- [x] 시간대 선택 슬라이더 (TimeSlider - 생활인구 시간별 데이터)

### ✅ 추가 구현 기능

- [x] 대기질 데이터 시각화 (PM10, PM2.5, O3, NO2, CO, CAI)
- [x] LOCALDATA 업종 데이터 (음식점, 병원, 약국 등 인허가 정보)
- [x] 복지시설 데이터 (노인/아동/장애인/정신보건/여성/일반사회복지시설)
- [x] 교통 정보 (CCTV 개수, 번호판영치 개수)
- [x] 계층형 지표 선택 UI (카테고리 > 주제 > 세부지표)
- [x] 구별 순위 사이드바 (서울시 전체 구 평균 비교)
- [x] 로딩 애니메이션 및 에러 상태 처리
- [x] 온보딩 투어 (OnboardingTour)
- [x] 랜딩 히어로 페이지 (LandingHero)
